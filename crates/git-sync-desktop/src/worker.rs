...
async fn run_repo(idx: usize, cfg: &RepoConfig, status_tx: Arc<watch::Sender<AppStatus>>) {
    ...
    // Run the push loop and the watcher concurrently. Both are inline futures so
    // they are cancelled automatically when run_repo itself is aborted — no
    // orphaned tasks can outlive this function.
